...
func BuildImageProxyURL(taskID string, idx int, ttl time.Duration) string {
	return BuildImageProxyURLWithBase("", taskID, idx, ttl)
}

// BuildImageProxyURLWithBase 生成图片代理 URL。
// baseURL 为空时返回相对路径;有值时返回绝对地址。
func BuildImageProxyURLWithBase(baseURL, taskID string, idx int, ttl time.Duration) string {
	return imageproxy.BuildURL(baseURL, taskID, idx, ttl)
}

func (h *ImagesHandler) imageProxyURL(taskID string, idx int, ttl time.Duration) string {
	baseURL := ""
	if h != nil && h.Settings != nil {
		baseURL = h.Settings.SiteAPIBaseURL()
	}
	return BuildImageProxyURLWithBase(baseURL, taskID, idx, ttl)
}
...
func (h *ImagesHandler) ImageProxy(c *gin.Context) {
	...
	if !imageproxy.VerifySig(taskID, idx, expMs, sig) {
